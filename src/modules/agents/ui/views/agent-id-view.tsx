...
export const AgentIdView = ({ agentId }: Props) => {
  // ✅ tRPC suspense hook — data is not undefined
  ...
  const removeAgent = trpc.agents.remove.useMutation({
    onSuccess: async () => {
      ...
      await utils.meetings.getOne.invalidate({ id: agentId });
      ...
      <div className="bg-white rounded-lg border">
        <div className="px-4 py-5 gap-y-5 flex flex-col col-span-5">
          ...
          <div className="flex flex-col gap-y-2">
            ...
            <p className="text-neutral-800 whitespace-pre-wrap">{data.instructions}</p>
          </div>
        </div>
      </div>
    </div>
